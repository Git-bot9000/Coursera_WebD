...
import { Breadcrumb, BreadcrumbItem,
            Button, Form, FormGroup, FormFeedback, Label, 
...
class Contact extends Component {
    ...
    handleSubmit(event) {
        ...
        alert("Curr state: "+ JSON.stringify(this.state));
    ...
    validate(firstname, lastname, telnum, email){
        ...
        if(this.state.touched.firstname && firstname.length<3)
            ...
            if(this.state.touched.firstname && firstname.length>10)
                errors.firstname='must be <=10 characters';    
        }
        if(this.state.touched.lastname && lastname.length<3)
            errors.lastname='must be >=3 characters';
        else{
            if(this.state.touched.lastname && lastname.length>10)
                errors.lastname='must be <=10 characters';    
        }

        const reg = /^\d+$/;
        if(this.state.touched.telnum && !reg.test(telnum))
            errors.telnum = 'must contain only numbers';
